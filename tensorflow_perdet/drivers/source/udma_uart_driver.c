...
uint16_t udma_uart_open (uint8_t uart_id, uint32_t xbaudrate) {
	...
	lDiv = (uint16_t)(5000000/xbaudrate);
	puart->uart_setup |= (lDiv << REG_UART_SETUP_DIV_LSB);
	//puart->uart_setup_b.div = (uint16_t)(5000000/xbaudrate);
	puart->uart_setup |= (3 << REG_UART_SETUP_BITS_LSB);
...
//	if (uart_id == 1)
	...
	puart->uart_setup |= (1 << REG_UART_SETUP_EN_TX_LSB);
	//puart->uart_setup_b.en_rx = 1;
	puart->uart_setup |= (1 << REG_UART_SETUP_EN_RX_LSB);
	
	//puart->uart_setup_b.rx_clean_fifo = 1;
	puart->uart_setup |= (1 << REG_UART_SETUP_RX_CLEAN_FIFO_LSB);
